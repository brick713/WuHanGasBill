...
# 关键：使用文档推荐的助手函数，它会在后台线程处理所有SSL阻塞I/O
...
class WuhanGasDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        """Fetch data from API."""
        try:
            async with async_timeout.timeout(15):  # 适当增加超时
    ...
    async def _fetch_balance(self):
        ...
        try:
            ...
            # 核心修正：使用 async_get_clientsession，不传递任何connector参数
            # 它将自动管理SSL上下文，避免阻塞I/O
    ...
    async def _fetch_annual_bills(self):
        ...
        try:
            headers = self._get_headers()
            # 核心修正：同样使用 async_get_clientsession
            session = async_get_clientsession(self.hass)
